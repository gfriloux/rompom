# Changelog — legacy (v0.15.0 and earlier)

**Frozen archive. Do not edit.**

These entries were written by hand before rompom adopted git-cliff. They carry much more
detail than the generated changelog can (worked examples, added files, migration notes), so
they are kept verbatim rather than regenerated from commit subjects.

Everything from **v0.16.0 onwards** lives in [`CHANGELOG.md`](./CHANGELOG.md) and is
generated by `just changelog`.
